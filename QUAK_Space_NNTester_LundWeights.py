...
def massArraySlicer(contents, binval, mass, loss1, loss2, masspoint, contentmax, binstoadd, useWSbins, wsbin, windowsize=500, nBins=50):
    ...
    for a in range(angularbincount+binstoadd-1):
        for r in range(radBins-1):
            if(contents[a,r]==contentsmin):
                ...
                if(contents[a,r]>0):
                    print('bin with ',contents[a,r], 'at', abins[a], rbins[r], 'x =', rbins[r]*np.cos(abins[a]), 'y =', rbins[r]*np.sin(abins[a]))
                contents[a,r]=newmax
    ...
    for b in range(len(nullXBounds)):
        ...
        if(len(justTestMass)>0):
            print('bin with ', len(justTestMass), 'events in SR at ', rbins[nullYBounds[b]]*np.cos(abins[nullXBounds[b]]), ',', rbins[nullYBounds[b]]*np.sin(abins[nullXBounds[b]]))
...
class QUAKRunner:
    def __init__(self, options, massvalue):
        ...
        self.aNN_name = options.aNN_name
    ...
    def makeH5s(self, jmevar=0):
        ...
        NNScore = branches[self.aNN_name]
        ...
        NNScore_bkg = branches2[self.aNN_name]
        ...
        NNScore = np.concatenate((NNScore, NNScore_bkg))
        ...
        massWindow = window_low & window_high
        massinwindow = mass[massWindow]
        nnscorewindow = NNScore[massWindow]

        #nnscorecut = np.sort(nnscorewindow)[-int(max(200, 100.*np.power((self.masspoint/1000.)-3, 4)))]
        #nnscorecut = np.sort(nnscorewindow)[-int(max(200, min(100.*np.power((self.masspoint/1000.)-3, 4), len(nnscorewindow))))]
        nnscorecut = np.sort(nnscorewindow)[-int(max(self.reqEvs, min(100.*np.power((self.masspoint/1000.)-3, 4), len(nnscorewindow))))]

        thefinalfinalbool = NNScore>nnscorecut
        massandcut = massWindow & thefinalfinalbool
        #np.logical_and(thefinalfinalbool, label==1)

        print('nnscorecut =', nnscorecut)
        print('we will look at', len(mass[thefinalfinalbool]), 'events.', len(mass[np.logical_and(thefinalfinalbool, label==1)]), 'of those are signal events.', len(mass[(np.logical_and(massWindow, thefinalfinalbool))]), 'events are in window, and', len(mass[np.logical_and(np.logical_and(massWindow, thefinalfinalbool), label==1)]), 'of those are signal events')
        themassarray = mass[thefinalfinalbool]
        ...
        hf.create_dataset('numEv', data=np.asarray([len(mass[thefinalfinalbool])]))
        ...
        print('number of events in histo =', np.sum(hist_testIn), 'number of signal events in window =', np.sum(hist_testIn_SIG))
        ...
        if(self.doWeightingTest):
            ...
            jmeselection2 = np.logical_or(JMEflag_forCut==-1, JMEflag_forCut==jmevar)
    ...
    def runFit(self):
        cmd = "python "+self.templateTop+"/dijetfit.py -i "+self.tagname+".h5 -M "+str(self.masspoint)+" --sig_shape "+self.templateTop+"/"+self.template+"/"+self.templateHeader+"_interpolation_M"+str(self.masspoint)+".0.root --dcb-model -p plots_M"+str(self.masspoint)+"_"+self.tagname+" --sig_norm_unc 0.15"
        if(not self.verbose):
            cmd = cmd+" > /dev/null"
        os.system(cmd)
...
#parser.add_option("-a", "--aNN_name", dest="aNN_name", default="NNScore_XYY235_Y170_Yp170_mjjAll")
parser.add_option("-a", "--aNN_name", dest="aNN_name", default="NNScore_XYY3000_Y400_Yp400_mjjAll")
